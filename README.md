...
For the data-centric improvement, we wanted to finetune an 'expert' doctor type model that could tutor med-school students. Sadly, this was not possible since we ran out of tokens. But we'll reason about what could have happened if we had trained this specialist model. The obvious expected result would be a massive improvement in the models ability to give clear and correct answers to medical questions, which would stand in stark contrast to our 'generalist' finetome models output. Another possible data centric improvement could have been using a larger amount of data or a different subset of the finetome dataset.

Another data-centric approach that we attempted was simply increasing the number of datapoints from the unsloth dataset from 5000 examples to 10000 (when training the 1B param llama models). However, this wasn't a controlled experiment since we changed the learning rate at the same time, meaning that we cant confidently claim that the the increase in performance (the increased coherence in the responses as can be see in the screenshots above) is due to the increase in data. Once again, we couldn't re-run the experiments to log the results of increased data due to lack of compute on colab, which is unfortunate, but we believe that an increase in data is going to lead to some improvements due to the fact that the model has more to learn from which can lead to more nuanced and coherent conversations.
